...
pub struct PaneProps {
    ...
    pub initial_left: f64,
    pub style_lhs: Option<AttrValue>,
    pub style_rhs: Option<AttrValue>,
}
...
pub fn Pane(props: &PaneProps) -> Html {
                ...
                Box::new(move |ev: Event| {
                    ...
                    if drag_state.is_some() {
                        document.remove_event_listener_with_callback("mousemove", drag_state.as_ref().unwrap().onmousemove.as_ref().unchecked_ref())
                            .expect("removeEventListener to have no error");
    ...
    html! {
        <div class="pane" ref={ pane_ref }>
            <div class="lhs" ref={ lhs_ref } style={ props.style_lhs.clone() }>{ lhs }</div>
            <div class="resizer" style={ format!("width: {}px;", RESIZER_WIDTH) }>
                <div class="line"/>
                <div class="handle" { onmousedown }/>
                <div class="line"/>
            </div>
            <div class="rhs" ref={ rhs_ref } style={ props.style_rhs.clone() }>{ rhs }</div>
        </div>
    }
}
